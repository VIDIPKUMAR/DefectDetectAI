...
PROJECT_ROOT = os.path.dirname(SCRIPT_DIR)  # Go up one level to k8s-ml-system
...
with st.sidebar:
            ...
            if health_response.status_code == 200:
                ...
                try:
                    ...
                    if stats_response.status_code == 200:
                        ...
                        with col1:
                            st.metric("Total", stats.get('total_processed', 0))
                        with col2:
                            st.metric("Defect %", f"{stats.get('defect_rate', 0):.1f}%")
                        with col3:
                            st.metric("Time", f"{stats.get('avg_processing_time', 0)}ms")
                except:
                    st.info("No stats endpoint available")
            else:
    ...
    # Quick test buttons - Using ABSOLUTE paths
    sample_images = {
        "Perfect": os.path.join(PROJECT_ROOT, "demo_perfect.jpg"),
        "Crack": os.path.join(PROJECT_ROOT, "demo_crack.jpg"), 
        "Scratch": os.path.join(PROJECT_ROOT, "demo_scratch.jpg"),
        "Defective": os.path.join(PROJECT_ROOT, "factory_defective.jpg")
    }
    
    # Debug: Show which files exist
    st.write("**File Status:**")
    for label, filepath in sample_images.items():
        exists = os.path.exists(filepath)
        status = "✅" if exists else "❌"
        filename = os.path.basename(filepath)
        st.write(f"{status} {label}: {filename}")
        if not exists:
            st.write(f"   *Looking for: {filepath}*")
    
    for label, filepath in sample_images.items():
        if st.button(f"Test {label}", key=f"btn_{label}"):
            try:
                st.write(f"**Debug: Trying to open:** `{filepath}`")
                ...
                if os.path.exists(filepath):
                    with st.spinner(f"Processing {label}..."):
                        with open(filepath, 'rb') as f:
                            ...
                            st.write(f"**Debug: Sending to API:** {filename}")
                            
                            response = requests.post(f"{st.session_state.api_url}/detect", 
                                                    files=files, 
                                                    timeout=10)
                            ...
                            if response.status_code == 200:
                                ...
                                st.success(f"✅ {label} sample analyzed!")
                                st.rerun()
                            else:
...
with tab1:
                ...
                if st.button("🚀 Analyze for Defects", type="primary", use_container_width=True):
                    with st.spinner("Processing image..."):
                        try:
                            ...
                            response = requests.post(f"{st.session_state.api_url}/detect", files=files, timeout=10)
    ...
    with col2:
        if 'last_result' in st.session_state:
            ...
            if defects > 0:
                st.error(f"❌ **REJECTED** - {defects} defects found")
            else:
                st.success("✅ **ACCEPTED** - No defects")
            
            # Metrics - Use .get() with defaults
            m1, m2, m3 = st.columns(3)
            with m1:
                st.metric("Defect %", f"{result.get('defect_percentage', 0):.2f}%")
            with m2:
...
with tab2:
    ...
    with col2:
        # Production stats - FIXED: Use numbers instead of strings with 'ms'
        stats_data = {
            'Metric': ['Total Processed', 'Accepted', 'Rejected', 'Avg Processing Time'],
            'Value': [12543, 11916, 627, 187]  # Changed '187ms' to 187
        }
...
with tab3:
    ...
    if st.session_state.history:
        history_data = []
        for entry in st.session_state.history[-20:]:  # Last 20 entries
            result = entry['result']
            history_data.append({
                'Time': datetime.fromisoformat(entry['timestamp']).strftime('%Y-%m-%d %H:%M:%S'),
                'File': entry['filename'],
                'Defects': result.get('defects_found', 0),
                'Defect %': f"{result.get('defect_percentage', 0):.2f}%",
                'Status': 'REJECTED' if result.get('defects_found', 0) > 0 else 'ACCEPTED',
                'Time (ms)': f"{result.get('processing_time_ms', 0):.0f}"
            })
    ...
    try:
        ...
        if health_response.status_code == 200:
            ...
            with col1:
                ...
                st.metric("API Status", "✅ Healthy" if status == 'healthy' else "⚠️ Warning")
            ...
            st.error(f"Cannot fetch system health: HTTP {health_response.status_code}")
    except Exception as e:
